...
import zyq.annotation.GenController;
...
public class GenControllerProcessor extends BaseCodeGenProcessor implements MethodCreator {
    ...
    public void genClass(TypeElement typeElement, RoundEnvironment roundEnvironment) {
        ...
        genJavaFile(generatePackage(typeElement), typeBuilder.build(), ann.sourcePath(), ann.overrideSource());
    }
    ...
    public String generatePackage(TypeElement typeElement) {
        return typeElement.getAnnotation(GenController.class).pkName();
    }


    @Override
    public Optional<MethodSpec> list() {
        if (StringUtils.containsNull(dtoPackageName, dtoClassName, voPackageName, voClassName, serviceName)) {
...
//                .addAnnotation(AnnotationSpec.builder(ApiOperation.class).addMember("value", "$S", tableName).build())
        ...
        CodeBlock context = CodeBlock.builder()
                ...
                .add("$T dataTable = getDataTable(list);\n", ClassName.get("com.zjhc.common.core.page", "TableDataInfo"))
                .add("Object[] fieldAndRemarks = $T.obtainFieldAndExcelAnnRemark($N.class);\n", excelUtil, voClassName)
