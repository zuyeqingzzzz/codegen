...
import zyq.context.CodeGenContext;
import zyq.holder.ProcessingEnvHolder;
import zyq.processor.impl.*;
...
public abstract class BaseCodeGenProcessor implements CodeGenProcessor {
    ...
    private void initContext(TypeElement typeElement) {
        ...
        context.setMapperPackageName("com.zyq.test.mapper");
        context.setMapperClassName("Mapper");
        context.setEntityPackageName("com.zyq.test.model");
        context.setEntityClassName("StaAreaTargetPc");
...
//            return List.class;
    ...
    protected void genJavaFile(String pkName, TypeSpec spec, String sourcePath, boolean overrideSource) {

        JavaFile javaFile = JavaFile.builder(pkName, spec).build();
        String dir = context.getProjectPath().replace("\\",File.separator) + File.separator + sourcePath.replace("/", File.separator);
